...
**Одиниця готовності — не гілка й не PR, а цільовий стан із макета власника:**
[design canvas «Оновлений каталог Fillando»](https://claude.ai/code/artifact/d6200740-5fe3-4edc-9461-b4fdf1ad08f0)
(14 екранів вітрини й адмінки). Макет складається з чотирьох джерел змін: TD-0002
(таксономія, кольори, лендінги), Фаза 0 (SEO-гігієна), TD-0006 (Google Merchant),
фікси аудиту (воронка замовлення + security).

Трекер приймання — [`docs/plans/plan-0005-catalog-target-state.md`](docs/plans/plan-0005-catalog-target-state.md).
Він, а не статус плану чи стан гілки, відповідає на питання «чи ми закінчили».

**Чотири різні стани, які не можна плутати в жодному звіті:**

| Стан | Що означає |
|---|---|
| `код у dev` | написано й змерджено в `dev`, покупець цього не бачить |
| `у проді` | `dev → main` задеплоєно, код працює на fillando.com |
| `дані готові` | міграції прогнані, контент написаний, ручні хвости розібрані |
| `видно покупцю` | усе вище разом; лише це рахується як зроблене |

**Правило формулювань.** Слова «готово», «завершено», «фінал» вживати тільки з
явною вузькою областю («PR-3 готовий», «гілка мержиться чисто») і ніколи — про
роботу загалом, доки трекер не зелений. Звіт про прогрес починати з позиції в
трекері (скільки пунктів досягнуто в проді), а не з обсягу написаного коду.
...
| **Що вважається зробленим** | `docs/plans/plan-0005-catalog-target-state.md` |
