...
| `COD` (Накладний платіж) | Тільки при `NOVA_POST` або `COURIER` |
...
- Номер замовлення
- Інформація про оплату, залежна від методу: `IBAN` — "Реквізити будуть надіслані на email", `CASH` — оплата при отриманні, `COD` — нагадування про часткову передоплату від 200 ₴ і дзвінок менеджера, `LIQPAY` — очікування підтвердження платежу
