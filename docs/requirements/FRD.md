...
1. **Продані товари** — агрегація по SKU за весь період: к-сть, у скількох
   замовленнях, середня ціна, сума. Сортування за сумою спадаюче. Суми —
   до знижок за промокодами.
...
- Якщо збережений `subtotal_price` вибірки розходиться з сумою позицій,
  з якої рахується розділ 1, — розбіжність друкується явно
