...
- **Status:** In Progress — PR-1…PR-7 **у проді** (гілка `production` обох репо; фід віддає 301 позицію на `api.fillando.com`, Merchant його читає). PR-8 (2026-09-22, назви й сімейства під Ads) — у `dev`, чекає на реліз. Лишаються: 33 (GA4 — блокер ремаркетингу), 31 (Search Console), 34, 40–41, і 35–36 після приймання. `☐` = не почато; `☑` = код у `dev`; `☐ прод` = чекає на реліз або на дію власника
- **Owner:** vvbogdanovih
- **Date:** 2026-09-06
...
### PR-8 (be) — фід під Google Ads (2026-09-22, після аудиту акаунта)

Аудит акаунта Google Ads за 22.08–20.09.2026 («fillando-google-ads-plan.pdf», розд. 3) показав,
що `title` фіда — це збережена назва варіанта без слів, які шукають. Правки локалізовані в
`google-shopping-feed.builder.ts`; міграцій і роботи в адмінці не потребують — усе вже лежить в
атрибутах.

| # | Task | Component | Depends on | Status |
|---|------|-----------|------------|--------|
| 37 | `title` = «{Категорія} {тип} {бренд} {діаметр} {вага} — {колір}»; тип — назва товару без бренду-префікса; частина, яку назва вже містить, не повторюється; одиниці з `Category.required_attributes[].unit` через `toPublicAttributes` | fillando-be | — | ☑ прод-код у `dev` |
| 38 | `g:product_highlight` з характеристик (діаметр, вага, `polymer`, `finish`, `reinforcement`, `series`, `spool_included`) | fillando-be | 37 | ☑ |
| 39 | `custom_label_0` = сімейство (`basic` / `decorative` / `engineering` / `flex`) замість назви категорії, однакової на всіх 301 позиції | fillando-be | — | ☑ |
| 40 | Реліз `dev → production` і **свідомий** «Отримати зараз» у Merchant: зміна назв відправляє всі 301 позицію на повторну модерацію (години — ~3 доби) | owner | 37–39 | ☐ прод |
| 41 | В Ads перерозбити групу товарів за `custom_label_0`, якщо вона була розбита за старим значенням («Філамент») | owner | 40 | ☐ прод |

Суха перевірка на живому каталозі (301 позиція): жодної назви понад 150 символів, жодного
дубля, середня довжина 43 → 54; сімейства — `basic` 150, `decorative` 93, `engineering` 44,
`flex` 14.
...
| # | Task | Component | Depends on | Status |
|---|------|-----------|------------|--------|
| 30 | Через адмінку встановити `google_product_category = 499682` для «Філамент» (§9 крок 11) | owner | 22, реліз | ☑ прод — поле є на всіх 301 позиції живого фіда (2026-09-22) |
| 31 | Search Console: верифікація, значення в `NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION` (§9 крок 13) | owner | 18, 20 | ☐ прод — meta-тега на проді немає (2026-09-22) |
| 32 | Merchant Center: акаунт, верифікація домену, URL фіда, розклад фетчу ≥ щогодини, account-level shipping **із `shipping-rates.json`**, return-policy 14 днів (§9 крок 14) | owner | реліз, 16 | ☑ прод — фід підключений: назви в звіті Ads збігаються з фідом (2026-09-22); розклад фетчу й shipping не перевірені з коду |
| 33 | Ads ↔ Merchant лінк; GA4-property, `NEXT_PUBLIC_GOOGLE_ANALYTICS_ID`; GA4 ↔ Ads лінк **без імпорту GA4 `purchase` як конверсії**; запуск PMax (§9 крок 15) | owner | 19, 20 | ☐ прод — **блокер**: у прод-білді фронту id порожній, усі GA4-події мовчать; без цього немає аудиторій ремаркетингу |
