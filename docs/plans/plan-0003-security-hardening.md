...
1. **`GET /products/price-sheet` — лишаємо публічним?** Це живий екран
   сайту, тож закриття = регрес UX. Рекомендація: лишити сторінку, але
   прибрати з відповіді **точний** залишок (лише boolean «є/немає»).
   Задачі 9 і 15 виконуються незалежно від відповіді; від неї залежить
   лише окрема правка формату відповіді. Деталі —
   [TD-0007 §8, питання 3](../designs/TD-0007-dealer-api.md).
